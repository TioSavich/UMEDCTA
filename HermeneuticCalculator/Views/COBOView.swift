...
struct COBOView: View {
    ...
    private func calculate() {
        ...
        var coboAutomaton = COBOAutomaton(a: a, b: b)
        coboAutomaton.run()

        // Update the state with the results from the automaton
        history = coboAutomaton.history
        jumps = coboAutomaton.jumps
        finalSum = coboAutomaton.finalSum
    }
}

struct COBOView_Previews: PreviewProvider {
