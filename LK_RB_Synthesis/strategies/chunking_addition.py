...
if __name__ == '__main__':
    ...
    A_test = 46
    B_test = 37
    chunking_auto = ChunkingAdditionAutomaton(inputs={'A': A_test, 'B': B_test})
    result = chunking_auto.run()
    
    print(f"\n--- {chunking_auto.metadata.strategy_name} History ({A_test} + {B_test}) ---")
    print(f"Final Result: {result}")
    
    if not chunking_auto.history:
        print("No history recorded.")
    else:
        df = pd.DataFrame(chunking_auto.history)
